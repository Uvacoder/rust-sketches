...
fn field(p: Vec2) -> Vec2 {
    ...
    let x = p.x + f * (p.y * u).atan().sin();
    let y = p.y + c * (p.x * v).tan().cos();
    vec2(x, y)
}
...
//             lin_srgb(0x52 as f32, 0x36 as f32, 0x34 as f32),
//             lin_srgb(0xb6 as f32, 0x45 as f32, 0x2c as f32),
...
//                 let point = point.rotate(TAU / 4.0);
...
//                 .xy(point)
//                 .w_h(2.0, 2.0)
//                 .color(gradient.get(map_range(length, 0.0, 2020.0, 0.0, 1.0)));
//         }
